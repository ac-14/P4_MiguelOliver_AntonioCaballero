...
if(method === "GET"){
  ...
  if(path === "/users"){
    ...
    const task = await tasksCollection.findOne({ _id: new ObjectId(task_id as string) });
    if (!task) {
      return new Response(JSON.stringify({
        error: true,
        status: 404,
    ...
    const destinationProject = await projectsCollection.findOne({ _id: new ObjectId(destination_project_id as string) });
    ...
    await tasksCollection.updateOne(
      { _id: new ObjectId(task_id as string) },
      { $set: { project_id: new ObjectId(destination_project_id as string) } }
    );
  
    const updatedTask = await tasksCollection.findOne({ _id: new ObjectId(task_id as string) });
